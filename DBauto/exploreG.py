from playwright.sync_api import sync_playwright
import re

with sync_playwright() as p:
    browser = p.chromium.launch(headless=True)
    page = browser.new_page(viewport={"width":1440,"height":900})
    page.goto("https://www.samsung.com/us/", timeout=60000, wait_until="domcontentloaded")
    page.wait_for_timeout(3000)
    page.evaluate("document.getElementById('consent_blackbar')?.remove()")
    page.wait_for_timeout(300)

    mobile = page.locator("a.nv00-gnb-v4__l0-menu-link", has_text="Mobile").first
    mobile.hover()
    page.wait_for_timeout(800)
    link = page.locator("a.nv00-gnb-v4__l1-menu-link", has_text="Galaxy Smartphones").first
    link.click()
    page.wait_for_load_state("domcontentloaded", timeout=60000)
    page.wait_for_timeout(3000)

    full = page.content()
    print("db-color-blue count:", full.count("badge-icon--db-color-blue"))
    print("bg-color-blue count:", full.count("badge-icon--bg-color-blue"))
    for m in re.finditer(r'data-product[a-zA-Z]*="[^"]*"', full):
        print(m.group())
        break
    ids = set(re.findall(r'data-product[a-zA-Z]*=', full))
    print(ids)
    with open("/root/work/AA/DBauto/full_page.html","w") as f:
        f.write(full)
    browser.close()
